...
def book_content(request,ref_code_book):
    ...
    list_capture = (
    Videos.objects
    .filter(ref_code=myurl, ref_code_video=ref_code_book)
    .order_by('chapter_title', 'id')  # ΠΡΟΣΟΧΗ: Πρέπει να ξεκινά με το πεδίο του distinct!
    .distinct('chapter_title')        # Λειτουργεί ΜΟΝΟ σε PostgreSQL
    .order_by('id')                   # Επαναταξινόμηση με βάση id
    .values_list('chapter_title', flat=True)
    )
