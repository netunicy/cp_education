...
def book_content(request,ref_code_book):
    ...
    list_capture = Videos.objects.filter(ref_code=myurl,ref_code_video=ref_code_book).order_by('chapter_title', 'id').values_list('chapter_title', flat=True).distinct('chapter_title')#διαγράφω τα dublicates
