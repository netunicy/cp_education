...
def book_content(request,ref_code_book):
    ...
    list_capture1 = Videos.objects.filter(ref_code=myurl,ref_code_video=ref_code_book).order_by('chapter_title').values_list('chapter_title', flat=True).distinct()#διαγράφω τα dublicates
    list_capture = list_capture1.all().order_by('id')
